...
for l = 1 : config.hierarchy_level
    ...
    if l == 1
        ...
        node_models{node_cnt} = train_node_model(Y, X, confusion_matrix, config, (1:length(Y)));
        node_models{node_cnt}.level = l;
        node_models{node_cnt}.right_indx = [];
        node_models{node_cnt}.left_indx = [];
        
        fprintf('mu has %d -2, %d 0, %d 1, %d -1 \n', ...
                sum(node_models{node_cnt}.mu == -2), sum(node_models{node_cnt}.mu == 0), ...
                sum(node_models{node_cnt}.mu == 1), sum(node_models{node_cnt}.mu == -1));
    else
        flag1 = false;
        flag2 = false;
        for k = 1 : length(node_models)
            ...
            if length(active_classes) >= 2
                ...
                if model_indx > 0
                    ...
                    disp(['training node ' num2str(node_cnt) '...']);
                    node_models{node_cnt} = train_node_model(Y(train_indx), X(train_indx, train_indx), ...
            ...
            if length(active_classes) >= 2 
                ...
                if model_indx > 0
                    ...
                    disp(['training node ' num2str(node_cnt) '...']);
                    node_models{node_cnt} = train_node_model(Y(train_indx), X(train_indx, train_indx), ...
                                                             confusion_matrix(active_classes, active_classes), ...
                                                             config, train_indx);
                    % update the mu
                    node_models{node_cnt}.mu = update_mu(active_classes, ...
                                                         num_classes,...
