...
{
	mexPrintf(
                  //	"Usage: model = svmtrain(training_label_vector,  'libsvm_options', training_instance_matrix);\n"
	"Usage: model = svmtrain(training_label_vector, training_instance_matrix, instance_margins, 'libsvm_options');\n"
	"libsvm_options:\n"
	...
	"-v n : n-fold cross validation mode\n"
        "-u flag: whether use user defined margins\n"
...
int parse_command_line(int nrhs, const mxArray *prhs[], char *model_file_name)
{
	...
	param.weight = NULL;
        param.margin = NULL; /* my defined margin for each instance*/
        param.use_defined_margin = 0; /* flag for user defined margin */
	cross_validation = 0;
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'q':
				...
				i--;
				break;
                        case 'u': /*use user defined margin*/
                                param.use_defined_margin = atoi(argv[i]);
                                break;
			case 'v':
				cross_validation = 1;
				nr_fold = atoi(argv[i]);
	...
	if(nrhs > 0 && nrhs < 5)
	{
		...
		if(mxIsSparse(prhs[1]))
		{
			...
			mexPrintf("Error: label vector and instance matrix must be double\n");
                    ...
                    param.margin = Malloc(double,prob.l);
                    double* margin = mxGetPr(prhs[3]);
                    memcpy(param.margin, margin, sizeof(double)*prob.l);
                    //                    mexPrintf("the first margin is %f\n", param.margin[0]);
                }
